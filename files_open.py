...
def open_file(file_name,mes_consulta):
    ...
    diccionario_valido = {'RFC':rfc_emisor, 'Tipo':tipo_Factura , 'Folio': serie_Factura + folio_Factura, 'Total': str_total, 'Fecha':fecha_timbrado, 'UUID':uuid_cfdi}
    if(fecha_timbrado[0:7] == mes_consulta):
        print(rfc_emisor + "\t" + tipo_Factura +"\t" +serie_Factura + folio_Factura + "\t" + str_total + "\t" + fecha_timbrado + "\t" + uuid_cfdi + "\t"+ relacionado_cfdi)
